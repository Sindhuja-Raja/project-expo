document.addEventListener('DOMContentLoaded', () => {
    ...
    const appendSaving = (dayCell, year, month, day) => {
        const savingKey = formatDate(year, month, day);
    ...
    const openSavingModal = (day, month = displayedMonth) => {
        ...
        const savingKey = formatDate(displayedYear, month, day);
    ...
    deleteAmountButton.addEventListener('click', () => {
        ...
        const savingKey = formatDate(year, month - 1, day);
    ...
    document.getElementById('saving-form').addEventListener('submit', (e) => {
        ...
        formData.append('date', savingDate);
        formData.append('amount', savingAmount);
    
        // Make the AJAX request to the backend (connect.php)
        fetch('connect.php', {
            method: 'POST',
            body: formData
        })
        ...
        .then(result => {
            console.log(result);  // Optional: show result from the backend
            renderCalendar(viewSelect.value);  // Re-render the calendar
            closeSavingModal(); // Close the modal after saving
        })
        .catch(error => {
